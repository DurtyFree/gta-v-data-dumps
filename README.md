...
## All data up2date as of GTA V update: **v3095 (Online 1.68) [DLC: mp2023_02_g9ec]**
---
...
---
## **Data dumps overview**
- **IPLs** ([ipls.json](ipls.json)) **805** ipls in total (Usable with IPL natives)
...
- **Particle Effects** ([particleEffectsCompact.json](particleEffectsCompact.json)) **339** particle effect dictionaries & **2681** particle effects in total (Usable with START PARTICLE FX natives)
...
- **Animations** ([animDictsCompact.json](animDictsCompact.json)) **19113** animation dictionaries & **250315** animations in total (Usable with TASK PLAY ANIM native)
...
- **(Offroad & Ped) Navigation Meshes** ([mega.nz MSGPACK download (.7z)](https://mega.nz/file/lLZWwJbR#T3Xm3nQKRTP1Z0XrkzgbbINo54d6wK2Na4Nj6tYfbXI), see [NAVIGATIONMESH.md](navigationmesh.md) for more info) **4883** navmeshes with **7281874** polygons in total (See navigationmesh.md for the messagepack model)
- **Ped Overlay Collections (Tattoos)** ([pedOverlayCollections.json](pedOverlayCollections.json)) **37** ped overlay collections with **3371** overlays in total (Usable tattos/badges with ped decoration native)
- **Timecycle Modifiers** ([timecycleModifiers.json](timecycleModifiers.json)) **1044** timecycle modifiers in total (Usable with TIMECYCLE MODIFIER natives)
...
- **Pickup Types names** ([pickupTypes.json](pickupTypes.json)) **173** pickup types in total (Usable with CREATE PICKUP natives)
- **Vehicle Mod Kits & Mods** ([vehicleModKits.json](vehicleModKits.json)) **534** vehicle mod kits with **30418** mods in total (Usable with SET_VEHICLE_MOD_KIT,SET_VEHICLE_LIVERY & SET_VEHICLE_MOD natives)
- **Vehicles** ([vehicles.json](vehicles.json)) **848** vehicle infos in total (Usable with VEHICLE natives)
- **AnimpostFX names** ([animPostFxNamesCompact.json](animPostFxNamesCompact.json)) **165** animpostfx names in total (Usable with ANIMPOSTFX natives)
- **Ped Component Variations** with cloth names and translations ([pedComponentVariations.json](pedComponentVariations.json)) **32167** component variations & **5617** ped props from a total of **92** ped component variation collections (Usable with COMPONENT VARIATION & PED PROP natives)
- **Ped Component Variations** without cloth names and translations ([pedComponentVariations_free.json](pedComponentVariations_free.json)) **32167** component variations & **5617** ped props from a total of **92** ped component variation collections (Usable with COMPONENT VARIATION & PED PROP natives)
- **Ped Apparel Restriction tag names** ([animPostFxNamesCompact.json](animPostFxNamesCompact.json)) **474** ped apparel restriction tags in total (Usable with PED RESTRICTION natives)
...
- **Vehicle Handlings** ([vehicleHandlings.json](vehicleHandlings.json)) **770** vehicle handling infos in total (Shared for all existing vehicles, see handling id in vehicles dump)
...
- **Static Emitters** ([staticEmitters.json](staticEmitters.json)) **863** static emitter infos in total (Usable with STATIC_EMITTER natives)
- **Ambient Zones** ([ambientZones.json](ambientZones.json)) **1375** ambient zone infos in total (Usable with AMBIENT_ZONE natives)
- **MLO / Interiors** ([mloInteriors.json](mloInteriors.json)) **367** MLO interiors at 819 locations in total (Useful for modding, also contains in game locations of interiors)
- **Peds** ([peds.json](peds.json)) **1054** peds in total (Useful for various natives related to peds)
...
- **Createable Objects** ([ObjectList.ini](ObjectList.ini) **19063** createable objects in total (The objects that can be created with most common loaded ytyps) (Useful for CREATE_OBJECT native)
...
- **Script Audio Bank names** ([scriptAudioBankNames.json](scriptAudioBankNames.json)) **598** script audio bank names in total (Usable with SCRIPT_AUDIO_BANK natives)
...
- **Cutscene names** ([cutsceneNames.json](cutsceneNames.json)) **771** cut scene names in total (Usable with CUT_ or CUTSCENE_ natives)
- **Weapons** ([weapons.json](weapons.json)) **180** weapons with **475** components in total (Usable with WEAPON natives)
...
- **Vending Machines** ([worldVendingMachines.json](objectslocations/worldVendingMachines.json)) **337** in total
- **Bin/Dumpster/Recycle Bins** ([worldBinsDumpsters.json](objectslocations/worldBinsDumpsters.json) & [worldRecycleBins.json](objectslocations/worldRecycleBins.json)) **7758** bins/dumpsters in total & **130** recycle bins in total
...
- **Public Phones** ([worldPublicPhones.json](objectslocations/worldPublicPhones.json) & [worldExtraPhones.json](objectslocations/worldExtraPhones.json)) **432** public phones in total & **317** extra phones in total
...
- **Fire Hydrants, Hoses & Drisers** ([worldFireHydrantDriser.json](objectslocations/worldFireHydrantDriser.json)) **1442** in total
...
- **Seats** (Benches, Chairs etc.) ([worldSeats.json](objectslocations/worldSeats.json)) **8028** in total
...
- **Cctv cameras** ([worldCctvs.json](objectslocations/worldCctvs.json)) **1641** in total
- **Electricity boxes** ([worldElectricityBoxes.json](objectslocations/worldElectricityBoxes.json)) **1905** in total
