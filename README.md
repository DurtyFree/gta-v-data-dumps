...
---
## **Data dumps overview**
- **IPLs** ([ipls.json](ipls.json)) **790** ipls in total (Usable with IPL natives)
...
- **Particle Effects** ([particleEffectsCompact.json](particleEffectsCompact.json)) **329** particle effect dictionaries & **2640** particle effects in total (Usable with START PARTICLE FX natives)
...
- **Animations** ([animDictsCompact.json](animDictsCompact.json)) **18772** animation dictionaries & **243134** animations in total (Usable with TASK PLAY ANIM native)
...
- **(Offroad & Ped) Navigation Meshes** ([mega.nz MSGPACK download (.7z)](https://mega.nz/file/lLZWwJbR#T3Xm3nQKRTP1Z0XrkzgbbINo54d6wK2Na4Nj6tYfbXI), see [NAVIGATIONMESH.md](navigationmesh.md) for more info) **4816** navmeshes with **7137375** polygons in total (See navigationmesh.md for the messagepack model)
- **Ped Overlay Collections (Tattoos)** ([pedOverlayCollections.json](pedOverlayCollections.json)) **36** ped overlay collections with **3305** overlays in total (Usable tattos/badges with ped decoration native)
- **Timecycle Modifiers** ([timecycleModifiers.json](timecycleModifiers.json)) **1021** timecycle modifiers in total (Usable with TIMECYCLE MODIFIER natives)
...
- **Pickup Types names** ([pickupTypes.json](pickupTypes.json)) **168** pickup types in total (Usable with CREATE PICKUP natives)
- **Vehicle Mod Kits & Mods** ([vehicleModKits.json](vehicleModKits.json)) **511** vehicle mod kits with **27746** mods in total (Usable with SET_VEHICLE_MOD_KIT,SET_VEHICLE_LIVERY & SET_VEHICLE_MOD natives)
- **Vehicles** ([vehicles.json](vehicles.json)) **815** vehicle infos in total (Usable with VEHICLE natives)
...
- **Ped Component Variations** with cloth names and translations ([pedComponentVariations.json](pedComponentVariations.json)) **31404** component variations & **5419** ped props from a total of **90** ped component variation collections (Usable with COMPONENT VARIATION & PED PROP natives)
- **Ped Component Variations** without cloth names and translations ([pedComponentVariations_free.json](pedComponentVariations_free.json)) **31404** component variations & **5419** ped props from a total of **90** ped component variation collections (Usable with COMPONENT VARIATION & PED PROP natives)
- **Ped Apparel Restriction tag names** ([animPostFxNamesCompact.json](animPostFxNamesCompact.json)) **478** ped apparel restriction tags in total (Usable with PED RESTRICTION natives)
...
- **Vehicle Handlings** ([vehicleHandlings.json](vehicleHandlings.json)) **747** vehicle handling infos in total (Shared for all existing vehicles, see handling id in vehicles dump)
...
- **Static Emitters** ([staticEmitters.json](staticEmitters.json)) **857** static emitter infos in total (Usable with STATIC_EMITTER natives)
- **Ambient Zones** ([ambientZones.json](ambientZones.json)) **1360** ambient zone infos in total (Usable with AMBIENT_ZONE natives)
- **MLO / Interiors** ([mloInteriors.json](mloInteriors.json)) **360** MLO interiors at 808 locations in total (Useful for modding, also contains in game locations of interiors)
- **Peds** ([peds.json](peds.json)) **1032** peds in total (Useful for various natives related to peds)
...
- **Createable Objects** ([ObjectList.ini](ObjectList.ini) **18335** createable objects in total (The objects that can be created with most common loaded ytyps) (Useful for CREATE_OBJECT native)
...
- **Cutscene names** ([cutsceneNames.json](cutsceneNames.json)) **757** cut scene names in total (Usable with CUT_ or CUTSCENE_ natives)
- **Weapons** ([weapons.json](weapons.json)) **177** weapons with **469** components in total (Usable with WEAPON natives)
